...
export default function SmokeDevicesScreen() {
  ...
  const currentDate = new Date('2025-03-03T07:43:39Z');
  ...
  const handleHeaderPress = () => {
    const deviceInfo = texts.devices.fireSprinkler;
    const welcomeMessage = `${texts.common.welcome} ${texts.common.user}. ${texts.common.dateTime}: ${formattedDate}. 
                          ${deviceInfo.category}: ${texts.devices.fireSprinkler.title}, ${texts.devices.siren.title}`;
      ...
      <Pressable onPress={handleHeaderPress}>
        <View style={[styles.headerSection, { backgroundColor: colors.surface }]}>
          ...
          <Text style={styles.categoryText}>
            {texts.devices.fireSprinkler.category}
          </Text>
        </View>
      </Pressable>

      {smokeDevices.map((device) => (
        <DeviceControl
          key={device.id}
          icon={device.icon}
